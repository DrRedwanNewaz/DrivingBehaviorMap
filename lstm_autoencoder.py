from keras.layers import Dense, Flatten, Input, merge, Dropout, Activation, Reshape, LSTM
...
def model_generator(latent_dim, input_shape, hidden_dim=90, reg=lambda: l1_l2(1e-5, 1e-5)):
    m = Sequential()
    m.add(LSTM(hidden_dim, input_dim=input(input_shape,), return_sequences=True))
    m.add(Dropout(reg()))
    m.add(LSTM(hidden_dim/2, return_sequences=True))
    m.add(Dropout(reg()))
    m.add(LSTM(hidden_dim / 4, return_sequences=True))
    m.add(Dropout(reg()))
    m.add(LSTM(latent_dim, activation=LeakyReLU(0.2)))
    return m
...
def driver_gan(path, adversarial_optimizer):
    ...
    def generator_sampler():
        zsamples = np.random.normal(size=(1, latent_dim))  #---------------------------------> (10,10)
